...
@Slf4j
public final class CORSFilter_old implements Filter {
    // ----------------------------------------------------- Instance variables
    /**
    ...
    public void doFilter(final ServletRequest servletRequest,
        ...
        log.debug("doFilter: "+ request.toString());
        
        // Determines the CORS request type.
        CORSFilter_old.CORSRequestType requestType = checkRequestType(request);

        // Adds CORS specific attributes to request.
        if (decorateRequest) {
            CORSFilter_old.decorateCORSProperties(request, requestType);
        }
        switch (requestType) {
        case SIMPLE:
    ...
    public void handleSimpleCORS(final HttpServletRequest request,
        ...
        CORSFilter_old.CORSRequestType requestType =
                checkRequestType(request);
        
        log.debug ("handleSimpleCORS");
        
        if (!(requestType == CORSFilter_old.CORSRequestType.SIMPLE
        || requestType == CORSFilter_old.CORSRequestType.ACTUAL)) {
            String message =
                    "Expects a HttpServletRequest object of type "
                            + CORSFilter_old.CORSRequestType.SIMPLE
                            + " or "
                            + CORSFilter_old.CORSRequestType.ACTUAL;
            throw new IllegalArgumentException(message);
        }

        final String origin =
                request.getHeader(CORSFilter_old.REQUEST_HEADER_ORIGIN);
        ...
        if (anyOriginAllowed && !supportsCredentials) {
            ...
            response.addHeader(
                    CORSFilter_old.RESPONSE_HEADER_ACCESS_CONTROL_ALLOW_ORIGIN, "*");
        } else {
        ...
        if (supportsCredentials) {
            response.addHeader(
                    CORSFilter_old.RESPONSE_HEADER_ACCESS_CONTROL_ALLOW_CREDENTIALS,
                    "true");
        }
        ...
        if ((exposedHeaders != null) && (exposedHeaders.size() > 0)) {
            ...
            response.addHeader(
                    CORSFilter_old.RESPONSE_HEADER_ACCESS_CONTROL_EXPOSE_HEADERS,
    ...
    public void handlePreflightCORS(final HttpServletRequest request,
        ...
        String accessControlRequestMethod =
                request.getHeader(CORSFilter_old.REQUEST_HEADER_ACCESS_CONTROL_REQUEST_METHOD);
        ...
        String accessControlRequestHeadersHeader =
                request.getHeader(CORSFilter_old.REQUEST_HEADER_ACCESS_CONTROL_REQUEST_HEADERS);
        ...
        if (preflightMaxAge > 0) {
            response.addHeader(
                    CORSFilter_old.RESPONSE_HEADER_ACCESS_CONTROL_MAX_AGE,
                    String.valueOf(preflightMaxAge));
        }

        // Section 6.2.9
        response.addHeader(
                CORSFilter_old.RESPONSE_HEADER_ACCESS_CONTROL_ALLOW_METHODS,
                accessControlRequestMethod);

        // Section 6.2.10
        if ((allowedHttpHeaders != null) && (!allowedHttpHeaders.isEmpty())) {
            response.addHeader(
                    CORSFilter_old.RESPONSE_HEADER_ACCESS_CONTROL_ALLOW_HEADERS,
    ...
    public void handleInvalidCORS(final HttpServletRequest request,
            final HttpServletResponse response, final FilterChain filterChain) {
        String origin = request.getHeader(CORSFilter_old.REQUEST_HEADER_ORIGIN);
        ...
        log.debug("handleInvalidCORS: " + message);
        log(message);
    }
    ...
    public static void decorateCORSProperties(final HttpServletRequest request,
        ...
        case SIMPLE:
            request.setAttribute(
                    CORSFilter_old.HTTP_REQUEST_ATTRIBUTE_IS_CORS_REQUEST, true);
            request.setAttribute(CORSFilter_old.HTTP_REQUEST_ATTRIBUTE_ORIGIN,
                    request.getHeader(CORSFilter_old.REQUEST_HEADER_ORIGIN));
            request.setAttribute(
                    CORSFilter_old.HTTP_REQUEST_ATTRIBUTE_REQUEST_TYPE,
        ...
        case PRE_FLIGHT:
            ...
            request.setAttribute(
                    CORSFilter_old.HTTP_REQUEST_ATTRIBUTE_REQUEST_HEADERS,
                    headers
                    );
            break;
        case NOT_CORS:
            request.setAttribute(
                    CORSFilter_old.HTTP_REQUEST_ATTRIBUTE_IS_CORS_REQUEST, false);
            break;
        default:
